...
const sessionSlotsForSelectedDate = computed((): TimeSlot[] => {
  if (!selectedDate.value) return []
  return slotsForDate(selectedDate.value)
})
...
onMounted(async () => {
  if (!user.value) {
    router.push('/auth/login?redirect=/member/admin/scheduling/attendance')
    return
  }
...
const selectDate = (date: Date) => {
  ...
  selectedSession.value = slots[0] ?? null
  loadBookingsForDate()
}
    ...
    <div v-if="isAdmin" class="px-4 py-6 max-w-2xl mx-auto">
      ...
      <div v-if="selectedDate" class="bg-gray-900 border border-gray-800 rounded-2xl p-4">
        ...
        <div class="flex gap-2 mb-4">
          <button
            v-for="slot in sessionSlotsForSelectedDate"
            :key="slot"
            @click="selectedSession = slot; loadBookingsForDate()"
            class="flex-1 py-3 rounded-xl font-semibold transition-all text-sm"
            :class="selectedSession === slot ? 'bg-gold-400 text-black' : 'bg-gray-800 text-gray-400'"
          >
