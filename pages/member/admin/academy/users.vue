...
const newUserForm = ref({
  ...
  full_name: '',
  phone: '',
  role: 'customer' as 'coach' | 'customer',
})
...
// Add user
const openAddModal = () => {
  const defaultRole = selectedRole.value === 'coach' ? 'coach' : 'customer'
  newUserForm.value = { email: '', password: '', full_name: '', phone: '', role: defaultRole }
  addUserError.value = ''
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-2">
              ...
              <button
                v-if="u.role === 'customer' && u.id !== user?.id"
                type="button"
                class="p-2 rounded-lg bg-red-900/30 text-red-400 hover:bg-red-900/50 transition-all disabled:opacity-50"
                :disabled="deletingUserId === u.id"
                :title="language === 'es' ? 'Eliminar patinador' : 'Delete skater'"
            ...
            <div>
              ...
              <div class="mb-3">
                <label class="block text-[10px] text-gray-500 mb-1">{{
                  language === 'es' ? 'Sesión grupal' : 'Group session'
                }}</label>
                <select
